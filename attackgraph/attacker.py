...
class Attacker(object):
    ...
    def att_greedy_action_builder(self, G, timeleft):
        ...
        mask = np.zeros(shape=(1, len(self.actionspace)), dtype=np.float32)
        canAttack = self.get_att_canAttack(G) + [1]
        ...
        while not isDup:
            ...
            if canAttack[x] == 0:
                isLegal = False
            if action == 'pass' or not isLegal or random.uniform(0, 1)<0.1:
                break
