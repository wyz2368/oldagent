...
class Environment(object):
    ...
    def step(self, action):
        if self.training_flag == 0: #defender is training.
            ...
            canAttack = self.attacker.get_att_canAttack(self.G) + [1]
            isLegal = True
            if canAttack[action] == 0:
                isLegal = False
            if true_action == 'pass' or true_action in self.attacker.attact or random.uniform(0,1)<0.1 or not isLegal:
                self.current_time += 1
